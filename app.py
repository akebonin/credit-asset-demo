...
DEFAULT_CONTRACT_ADDRESS = "0x35750342f1A55E8F6B799E3cD1129d6e4Df7c3B5"
...
try:
    if mode == "🛠 Developer/Test Mode":
        rpc_url = st.sidebar.text_input("🔗 RPC URL", value=f"https://sepolia.infura.io/v3/{INFURA_KEY}")
        contract_address = st.sidebar.text_input("📬 Contract Address", value=DEFAULT_CONTRACT_ADDRESS)
    else:
        rpc_url = f"https://sepolia.infura.io/v3/{INFURA_KEY}"
        contract_address = DEFAULT_CONTRACT_ADDRESS

    w3 = Web3(Web3.HTTPProvider(rpc_url))  # ← FIXED assignment here
    ...
    if contract:
        ...
        if mode == "🛠 Developer/Test Mode" and consent:
            ...
            if st.button("Trigger releaseFunds()"):
                st.warning("On-chain write not implemented in demo (wallet signing required).")
...
with tab2:
    ...
    try:
        ...
        asset_types = st.sidebar.multiselect("Select Asset Type(s):", df["asset_type"].unique(), default=df["asset_type"].unique())
        ...
        fig1 = px.scatter(filtered_df, x="yield_prediction", y="credit_score", color="region",
                          size="soil_moisture", hover_data=["farm_id", "asset_type"])
